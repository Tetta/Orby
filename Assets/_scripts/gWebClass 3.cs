...
public class gWebClass3 : MonoBehaviour {
	...
	void Start () {
		...
		cloth = rope.GetComponent<InteractiveCloth> ();
		cloth.externalAcceleration = new Vector3(0, -300, 0);
		cloth.AttachToCollider(berry.transform.collider, false, true);
	...
	void OnMouseDown () {
		...
		if (webState == "enableWeb") {
			...
			webState = "";
			cloth.DetachFromCollider(berry.transform.collider);
			Debug.Log(3);
			//globalCounter = 1;
		} 
	}
